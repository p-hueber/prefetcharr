...
- Improve support for unprivileged users in Docker.
...
- New experimental config option `append_to_queue` (default off). When enabled,
  prefetcharr appends the upcoming episodes to the player's active play queue on
  the media server. Not fully supported by all clients. Not possible with
  Tautulli.
...
- Add support for Tautulli as a way to connect to Plex. ([@daniel-tp](https://github.com/daniel-tp))
